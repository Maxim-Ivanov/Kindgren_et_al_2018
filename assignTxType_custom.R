# The original assignTxType() function from CAGEfightR 0.2.0 was modified to remove an excessive category in the annotation hierarchy. Namely, the CDS category was removed (otherwise it did consume the vast majority of exonic and intronic TSS)

assignTxType_custom <- function (gr, txdb, tssUpstream = 100, tssDownstream = 100, proximalUpstream = 1000, asFactor = FALSE) {
  message("Running a custom version of assignTxType (CDS annotation was removed)...")
  ...
  if (asFactor) {
    featureType <- factor(featureType, levels = c("intergenic", "proximal", "promoter", "fiveUTR", "intron", "exon", "threeUTR", "PROMPT", "antisense"))
  }
  featureType
}